...
def num_segregating_sites(char_array, ignore_uncertain=True):
    """
    Returns the raw number of segregating sites (polymorphic sites).
    """
    return _num_segregating_sites(char_array.vectors(), char_array.default_state_alphabet, ignore_uncertain)

def average_number_of_pairwise_differences(char_array, ignore_uncertain=True):
    """
    Returns $k$, calculated for a character block.
    """
    return _average_number_of_pairwise_differences(char_array.vectors(), char_array.default_state_alphabet, ignore_uncertain)

def nucleotide_diversity(char_array, ignore_uncertain=True):
    """
    Returns $\pi$, calculated for a character block.
    """
    return _nucleotide_diversity(char_array.vectors(), char_array.default_state_alphabet, ignore_uncertain)
...
def wakeleys_Psi(char_array, taxon_groups, ignore_uncertain=True):
    """
    ...
    char_x = []
    char_y = []
    state_alphabet = char_array.default_state_alphabet
    for t in char_array.taxon_set:
        if t in taxon_groups[0]:
            char_x.append(char_array[t])
        else:
            char_y.append(char_array[t])
    ...
    ax = float(n_x * (n_x - 1))
    ay = float(n_y * (n_y - 1))
    k = average_number_of_pairwise_differences(char_array, ignore_uncertain)
    psi = (float(1)/(a)) * ( ax * (math.sqrt(s2_x)/d_x) + ay * (math.sqrt(s2_y)/d_y) + (2 * n_x * n_y * math.sqrt(s2_xy)/k))
    return psi

def summarize(char_array, taxon_groups, ignore_uncertain=True):
    """
    ...
    # Hickerson 2006: S #
    summary["S"] = num_segregating_sites(char_array, ignore_uncertain)

    # Hickerson 2006: theta #
    a1 = sum([1.0/i for i in range(1, len(char_array))])
