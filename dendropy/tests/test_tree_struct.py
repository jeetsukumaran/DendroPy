...
def build_fixed_basic_tree1():
    ...
    node_B = tree.seed_node.new_child(oid='B')
    node_B.new_child(oid='A')
    node_D = node_B.new_child(oid='D')
    node_D.new_child(oid='C')
    node_D.new_child(oid='E')
    node_G = tree.seed_node.new_child(oid='G')
    node_I = node_G.new_child(oid='I')
    node_I.new_child(oid='H')
    tree.debug_check_tree(_LOG)
    return tree

def build_fixed_basic_tree2():
    """
    ...
    node_i1 = tree.seed_node.new_child(oid='i1')
    ...
    tree.seed_node.new_child(oid='T14')
    node_i10 = tree.seed_node.new_child(oid='i10')
...
class TreeTraversalChecker(unittest.TestCase):
    ...
    def setUp(self, tree1=None, tree2=None, title=None, set_oids_to_labels=False, logger=None):
        self.tree1 = build_fixed_basic_tree1() if tree1 is None else tree1
        self.tree2 = build_fixed_basic_tree2() if tree2 is None else tree2
        if set_oids_to_labels:
            for t in [tree1, tree2]:
                for nd in t.postorder_node_iter():
                    if nd.taxon is not None:
                        nd.oid = nd.taxon.label
                    elif nd.label is not None:
                        nd.oid = nd.label
        self.title = "Programmatically-Composed Tree" if title is None else title
    ...
    def checkLeafIter(self):
        self.logger.debug('### LEAF ITERATION: %s ###' % self.title)
        result = [node.oid for node in self.tree1.leaf_iter()]
        expected = ["A", "C", "E", "H"]
        self.logger.debug("EXPECTED: %s" % (', '.join(expected)))
        self.logger.debug("  RESULT: %s" % (', '.join(result)))
        assert result == expected
        result = [node.oid for node in self.tree2.leaf_iter()]
        expected = ["T1", "T2", "T3", "T4", "T5", "T6", "T13", "T7", "T8", "T9", "T10", "T14", "T11", "T12"]
        self.logger.debug("EXPECTED: %s" % (', '.join(expected)))
        self.logger.debug("  RESULT: %s" % (', '.join(expected)))
        assert result == expected

    def checkPreorder(self):
        self.logger.debug('### PRE-ORDER ITERATION: %s ###' % self.title)
    ...
    def checkPostOrder(self):
        self.logger.debug('### POST-ORDER ITERATION: %s ###' % self.title)
    ...
    def checkLevelOrder(self):
        self.logger.debug('### LEVEL-ORDER ITERATION: %s ###' % self.title)
    ...
    def runChecks(self):
        ...
        self.checkPreorder()
        self.checkPostOrder()
        self.checkLevelOrder()

class TreeTraversalRunner(TreeTraversalChecker):
...
#         d = Dataset()
#         tree = d.trees_from_string(str=n, format="NEWICK")[0]
