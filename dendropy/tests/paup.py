...
class PaupWrapperDumbTests(unittest.TestCase):
    ...
    def setUp(self):
        # each element = ( ( <tree file>, [<taxa block file> | None] ),
        #                  ( <number of trees in file> ),
        #                  ( { split_string : (tree count, percentage} ) )
    ...
    def check_group_freqs(self, treefile, taxafile, exp_tree_count, group_freqs):
        ...
        p = Paup()
        commands = []
        if taxafile is not None:
            taxafile = dendropy.tests.data_source_path(taxafile)
        ...
        assert exp_tree_count == tree_count
    ...
    def testGroupFreqs(self):
        for i in self.split_test_cases:
            self.check_group_freqs(i[0][0], i[0][1], i[1], i[2]) 
