...
class PureBirthModelEstimationTestCase(unittest.TestCase):
    ...
    def testBasicEst(self):
        ...
        for tree, expected_result in zip(trees, expected_results):
            obs_result = birthdeath.fit_pure_birth_model(tree.internal_node_ages(check_prec=1e-5))
            # print("{} == {},\t\t{} == {}".format(obs_result[0], expected_result[0], obs_result[2], expected_result[1]))
