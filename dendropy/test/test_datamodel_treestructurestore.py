...
class TreeStructureStoreBasicTreeAccession(unittest.TestCase):
    ...
    def verify_tree_structure_store(self, tree_structure_store, source_trees):
        self.assertEqual(len(tree_structure_store), len(source_trees))
        for idx, source_tree in enumerate(source_trees):
            source_tree.encode_splits()
            source_splits = list(source_tree.split_edge_map.keys())
            tss_splits, tss_edges = tree_structure_store[idx]
