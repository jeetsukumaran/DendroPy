...
def reroot_on_lowest_common_index_path(t, common_mask):
    ...
    if (curr_n.edge.split_bitmask & common_mask) == l:
        ...
        t.to_outgroup_position(curr_n, update_splits=True, collapse_root_bifurcation=True)
        avoid = curr_n
                ...
                if curr_n is not avoid:
                    ...
                    if cm:
                        if cm == without_lowest:
                            ...
                            t.reseed_at(curr_n, update_splits=True, collapse_root_bifurcation=True)
                            t.to_outgroup_position(r, update_splits=True, collapse_root_bifurcation=True)
    ...
    if curr_n is not t.seed_node:
        ...
        assert(len(children) > 1)
        p = curr_n.parent
        t.to_outgroup_position(children[0], update_splits=True, collapse_root_bifurcation=True)
        t.to_outgroup_position(p, update_splits=True, collapse_root_bifurcation=True)
    else:
        # if the root first relevant, node then we just make the path leading
        #   to the lowest index node the first child of the root
        t.to_outgroup_position(lowest_on_path_to_l, update_splits=True, collapse_root_bifurcation=True)
